class Merchant < ActiveRecord::Base
  ...
  def self.top_merchants_by_number_sold(quantity)
    ...
    Merchant.select("merchants.*, count(invoice_items.quantity) AS item_sold").joins(invoices: :invoice_items).where("transactions.result = ? AND invoice_items.item_id = ?", "success", "").group("merchants.id").order("item_sold DESC").take(quantity)
